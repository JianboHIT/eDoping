...
def cmd(arg=None):
    ...
    parser_query.add_argument('-c', '--compround', help='Assign the target compround')
    ...
    parser_query.add_argument('element', nargs='+', help='Symbol of the chemical element of the system')
    ...
    if args.task is None:
        ...
        if not is_quiet:
            print(f'Searching for phases with elements: {" ".join(args.element)}')
        phases = query_oqmd(elements=args.element,
        ...
        target = args.compround
        if target is None:
            outs.append(dsp.format(*('X' * (len(elements)+1))))
        else:
            target = get_comp(target)
            outs.append(dsp.format(*[target.get(e, 0) for e in elements], 'X'))
        
        with_struct = args.structure
        for phase in phases:
            comp = get_comp(phase['name'])
            ratios = [comp.get(e, 0) for e in elements]
        ...
        with open(args.output, 'w') as f:
            f.writelines(outs)
        if not is_quiet:
            print(f'Data saved to {args.output}. (DONE)')
