    ...
    if args.task is None:
        ...
        if args.transform:
            if args.transform == 'TRANSMAT.in':
                trans_mat = np.loadtxt('TRANSMAT.in', skiprows=1, usecols=(0,1,2), max_rows=3)
                if is_detail:
                    print('Read transformation matrix from TRANSMAT.in file.')
            else:
                try:
                    trans_mat = np.array([float(v) for v in args.transform.split()])
                except ValueError:
                    eg = "'2 0 0 0 2 0 0 0 2', '2 2 2', or '2'"
                    raise ValueError(
                        "Invalid transformation matrix: '{}' (allowed: {})".format(args.transform, eg)
                    ) from None
                ...
                if nsize == 1:
                    ...
                    raise ValueError(
                        'Invalid transformation matrix size: {} (allowed: 9, 3, 1)'.format(nsize)
                    )
            ...
            np.savetxt('TRANSMAT', trans_mat, fmt='%6d', header=dsph, comments='')
            if is_quiet:
            ...
            site_, dx_, dy_, dz_ = args.cubicize
            ...
            if site:
                ...
                if is_detail:
                    print('Displace {}{} by ({:.4f}, {:.4f}, {:.4f}), {:.4f} Angstroms'.format(atom, idx, *dr_cart, distance))
                elif is_quiet:
                    print('{:.4f}'.format(distance))
                else:
                    print('Displace {}{} by {:.4f} Angstroms'.format(atom, idx, *dr_cart))
                dr_frac = dr_cart @ np.linalg.inv(pos.basis)
                pos[atom][idx-1] += dr_frac
            else:
