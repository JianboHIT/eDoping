...
def cmd(arg=None):
    ...
    parser_exclusive.add_argument('-v', '--verbosity', action='count', default=0, help='increase output verbosity')
    parser_exclusive.add_argument('-q', '--quiet', action='store_true', help='only show key output')
    ...
    parser_refine_exclusive = parser_refine.add_mutually_exclusive_group()
    parser_refine_exclusive.add_argument('-t', '--transform', nargs='?', const='TRANSMAT.in', metavar='TRANSMAT', help='Apply the transformation matrix (default: read from TRANSMAT.in)')
    ...
    if args.task is None:
        ...
        if args.transform:
            ...
            if is_quiet:
                ...
                print(f'Strain tensor (det = {volume_ratio:.4f}):')
                print(strain_tensor)
            else:
