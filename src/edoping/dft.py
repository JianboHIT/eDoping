...
class _Cell():
    ...
    def write(self, poscar='POSCAR.new', ptype='vasp'):
        ...
        sites = self.sites
        lines = []
        lines.append('POSCAR generated by RDF')
        lines.append(f'{1:8.3f}\n')
        for basis_i in basis:
            line = [f'{basis_ii:22.16f}' for basis_ii in basis_i]
            lines.append(''.join(line)+'\n')
        ...
        for elt, site in sites.items():
            for idx, pos in enumerate(site, start=1):
                f_abc = [f'{pos_i:22.16f}' for pos_i in pos]
                label = f'{elt+str(idx):>7s}'
                lines.append(''.join(f_abc)+label+'\n')
        with open(poscar, 'w') as f:
            f.writelines(lines)
    
    def pop(self, atom, index=0, clean=True):
        '''
        Remove atom
        '''
        if atom in self.sites:
            self.sites[atom].pop(index)
            if clean and (len(self.sites[atom]) == 0):
                del self.sites[atom]
        else:
            raise RuntimeError(f'Failed to locate {atom}')
    
    def insert(self, atom, pos, index=0, tohead=True):
        if atom in self.sites:
            self.sites[atom].insert(index, pos)
        else:
            self.sites[atom] = [pos,]
            if tohead:
